...
async function testShawnaRealQuote() {
  ...
  const pdfPath = "./tests/project-quotes/required_first_utility_bill_shawna.pdf";
  ...
  try {
    ...
    console.log("  Match:", depositMatchPercent < 10 ? "✅ WITHIN 10%" : "⚠️ OFF BY " + depositMatchPercent.toFixed(1) + "%");
    ...
    if (priceMatch && depositMatchPercent < 10) {
      ...
      if (!priceMatch) console.log("  - Electricity price extraction: " + (priceDiff * 100).toFixed(2) + " cents off");
      if (depositMatchPercent >= 10) console.log("  - Protocol deposit: " + depositMatchPercent.toFixed(1) + "% variance");
    }
