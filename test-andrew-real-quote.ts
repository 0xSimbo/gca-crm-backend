...
async function testAndrewRealQuote() {
  ...
  const projectData = {
    ...
    annualConsumptionMWh: (10528 / 1000).toString(), // 10.528
    numberOfPanels: 22,
    startDate: "09/01/2025",
    expectedElectricityPrice: 0.1075, // $0.1075/kWh from spreadsheet
    expectedProtocolDeposit: 18658.3, // From spreadsheet
  };
  ...
  const pdfPath =
    "./tests/project-quotes/required_second_utility_bill_andrew.pdf";
  ...
  try {
    ...
    if (priceMatch && depositMatchPercent < 5) {
      console.log(
        "✅ ALL CHECKS PASSED - Implementation matches expected values!"
      );
    } else {
      console.log("⚠️ Some variances detected:");
      if (!priceMatch)
        console.log("  - Electricity price extraction needs review");
      if (depositMatchPercent >= 5)
        console.log("  - Protocol deposit calculation has variance");
    }
  } catch (error) {
